...
| O-13 | 多段倍率（0.55×2）各段是否分别过命中/暴击/护盾 | 默认"各段独立结算实例"（独立暴击与事件记录）；护盾粒度 M4 接入前不影响（M2 无护盾），留粒度开关 | 05 |
| O-14 | 士气团队性事件聚合口径（暴击+5/击杀+10/队友虚弱−8/死亡−15） | 默认"实例各自结算、全队性事件同动作内合并一次"（组合行待确认见 §6） | 08 / 09 / 10 |
...
> 超出既有编号的新疑问（如"精神 AOE + 暴击的士气取值""虚弱者受精神伤害时 −8 与 −5 是否叠加""跨阵营同速破平局先后"）本文件不占用编号，仅列 §6"组合行待确认"，由架构师按 _conventions §5 登记（O-31 起）后回填。
...
- **风险/开放**：O-02（是否档内随机，待实测，不阻塞）；眩晕 buff 数据位在 buff_defs（M3/M4），M2 只读标记接口预留；**跨阵营同速先后**文档未定义 → §6 组合行待确认（M2 默认"我方先于敌方"并锁单测）。
...
- **决策**：#154（敌人 HP 上调 50/50/38/34，已入 units.json）
...
- **要点**：
  ...
  3. **§7.2 回合数试算（~9 回合）不属 M2 验收**：由 M6 Monte Carlo（≥300 场，胜率 40~70%，verification §2）验证；M2 只提供公式确定性基础与敌人 HP 起手值（#154 已入 units.json）。
...
## 6. 组合行待确认（不入 open_issues 编号，需策划回读）
...
> 以上三项均为确定性管线必须给出固定语义的边界，本文件给"M2 默认"并标注可推翻；建议架构师在 open_issues.md 登记（O-31 起），本文件不占用编号。
