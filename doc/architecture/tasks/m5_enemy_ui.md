...
- 开放题一律引用 open_issues.md §B 权威编号：**O-11（#169 已定撤退公式）/ O-19（AI 15% 随机开关）/ O-20（增援强度与 +攻/+速数值）/ O-21（#170 已定：显式 −4 取代 −8、目标位为准）/ O-28（射手 AI 规则序与贴脸口径）/ O-30（敌人意图显示可选）**；另涉 **O-31（威吓箭表达式已修正 13×0.5，enemy.md）** 与 **O-38（#178/#179：单体=候选池选一 → M5 需 ③b 目标选择步 + AI 选目标）**。
...
| M-C | 撤退按钮显示成功率数字 | 04 | 按钮数字 == **#169 公式（tuning `retreat_formula`：50%+速度差×4% 钳[15,85]、±10 扰动钳[5,95]）**在当回合速度/存活快照下的输出；击杀最快敌人后数字实时变化 |
...
- **要点**：
  ...
  2. **属性起手值（enemy §5.2 表，逐字段入 units.json，敌方 `deaths_door_resist` 必须为 `null`——敌方无死门/士气/虚弱/崩溃系统，P7 校验）**：
     ...
     |---|---|---|---|---|---|---|---|---|---|---|---|---|
     | 近战小兵 `melee_soldier` | 60 | 12 | 8 | 8 | 10 | 5 | 55 | 30 | 30 | 25 | 55 | null |
     | 远程射手 `ranged_archer` | 46 | 13 | 4 | 12 | 15 | 8 | 60 | 25 | 30 | 25 | 25 | null |
     | 施法者 `caster` | 41 | 12 | 3 | 9 | 10 | 5 | 70 | 40 | 30 | 30 | 30 | null |
...
- **完成判据（可测）**：
  - units.json 敌方 3 行与 enemy §5.2 逐字段一致（含速度 8/12/9、位移抗 55/25/30、**HP 60/46/41（#171 生效值）**），校验断言 `deaths_door_resist == null`（P7）通过。
...
- **风险/开放**：O-06（敌方 **HP 生效值 60/46/41（#171/#173）**，可推翻、重基线后再核）；O-31（威吓箭表达式已由 enemy.md 修正为 13×0.5，与 skills.json 0.5 倍率自洽，**已关闭**）；blueprint §3 数据文件名现已与 data_schema §1 对齐（见 §0）。
...
- **要点**：
  ...
  7. **目标选择与 taunt（#178/#179，v0.44）**：技能只选**位置**（glossary §2），但位置范围是**候选池**——单体技能（无 `aoe`：重劈/精准射击/威吓箭）在候选池内**确定性地选一个**非空目标（槽位编号升序首个非空、不含 RNG）；AOE（精神震荡）结算范围内全部非空（无选择）；自身/位移类技能无目标选择。`taunt` 打断：若该敌身上有 `taunt` buff（data_schema buff_defs `taunt`，2 回合）且嘲讽者所在位在其技能候选池内 → 该位优先（"改敌方 AI 优先攻击施法者"）。注：施法者"点名最低韧性"与固定 target"我 1 位"冲突已按 **#170/O-21 裁定**——以技能目标位为准（当前恐惧低语 target=[1] 即"打我 1 位"），AI 文字描述不得推翻目标位。
...
- **完成判据（可测）**：
  ...
  - 开关测试：`random.enabled=true` 时开启 15% 分支的抽取带 DrawCount 且入 CombatLog；default off 下 100 次 Choose 无一走次优先。
   - **单体选一断言（#178/#179）**：精准射击（target 我 1~4 多非空、无 aoe）→ Choose 输出**恰好 1 个目标槽**（默认槽号升序首个非空；`taunt` 位优先）；同快照两次 Choose 结果一致（确定性）；威吓箭同口径（目标我 1~2 内选一）。
...
- **产出**：
  ...
  - `res://data/tuning.json`（`retreat`：success_morale/fail_morale + **#169 公式常量与默认示例：tuning `retreat_formula`**）
...
- **要点**：
  ...
  2. **判定 = 混合式（#169 已定，O-11 关闭）**：`基础成功率 = 50% + (我方平均实际速度 − 敌方平均实际速度) × 4%`，钳 **[15%,85%]**；`实际 = 基础 + uniform(−10,+10)`，钳 **[5%,95%]**（GDD §3.6 混合式 + #46 ±10% 随机）；公式与默认数值落 tuning `retreat_formula`（代码不写死，策划可回填）。随机取数走 IRngProvider 固定调用点（blueprint §8.2 撤退层）。
...
- **完成判据（可测）**：
  - 按钮数字 == tuning `retreat_formula`（#169）在当回合实际速度/存活单位快照下的输出（单元测试直接调同一公式函数比对）；杀敌后数字单调变化（移走最高速敌方则提高，可断言方向）。
...
- **风险/开放**：O-11 已关闭（#169 公式定案）；ui_spec §9"撤退成功率是否实时"建议实时已采纳（显示层口径，非数值决策）。
...
- **要点（9 条必显示逐条落地：实现要点 + 出处 + 观测判据）**：
  ...
  | 2 | **撤退成功率具体数字** | **#169 公式（tuning `retreat_formula`）** + 当回合实际速度快照，A 区按钮常驻显示（如"撤退 68%"）；实时刷新（T-M5-04 要点 5） | GDD §3.6 / ui_spec 必显 #2 | 数字 == #169 公式输出；杀敌后刷新 |
  ...
  | 5 | **目标范围高亮** | skills.json `target`（slots 语义）+ 位置编号映射 = **候选池（#178）**：范围内**部分非空高亮全部非空位**（均可选）、全空灰显；**单体技能 → 高亮候选后玩家点选其一（③b）**；AOE → 整体高亮、不选单个（ui_spec §4 ③b） | ui_spec 必显 #5 / GDD §1.2 | 选中技能后高亮位置 == target.slots ∩ 非空位；③b 选中项高亮 == 玩家点选槽 |
...
- **要点（操作流 ①~⑥，ui_spec §4）**：
  ...
  3. ③ 显示目标范围高亮 + 位移预览（整条交换链）+ 命中率（T-M5-06 #4/#5/#6）；**③b（#178 / ui_spec §4）【仅单体技能】在候选池内点选具体目标**：多个非空 → 玩家点选其一，选中项高亮 + 显示该目标命中率/伤害预估/位移预览；仅 1 个非空 → 自动选中可直接确认；AOE 跳过 ③b（整体一起结算）。**可取消重选不消耗行动**（④ 确认前任意取消/换技能，不发任何 BattleCommand）。
...
- **完成判据（可测）**：
  - 手工走查 ①~⑥ 全链一次（选可用技能 → ③b 目标选择（单体）→ 预览 → 确认 → 五类反馈依次出现）；③ 阶段取消 3 次后再确认，BattleDirector 收到的命令数 = 1（不耗行动断言）；
  - ③b 断言（#178）：单体技能候选 ≥2 时未点选不能确认（确认按钮仅在高亮候选被选中后可用）；AOE 技能不出现目标选择步。
...
| ID | 议题 | 本文件落点 |
|---|---|---|
| O-11 | 撤退成功率公式（#169 已定） | T-M5-04：tuning `retreat_formula`（50%+速度差×4% 钳[15,85]、±10 扰动钳[5,95]），按钮数字与公式一致（M-C） |
...
| O-21 | 威吓箭 −4 vs 精神 −8 / 施法者点名最低韧性 vs 固定"我 1 位" | ✅ 已定（#170）：T-M5-01/02 按显式 −4 + 目标位为准实施 |
| O-31 | 威吓箭伤害表达式 vs 射手攻击 13 | ✅ 已关闭：enemy.md 已修正 13×0.5，T-M5-01 数据自洽 |
| O-28 | 射手 AI 规则②使③威吓箭不可达 / 贴脸口径 | T-M5-02：按 data_schema §3.5 表序直译，不静默改序 |
| O-30 | 敌人意图 / 伤害预估显示可选 | T-M5-06 注：可选项，建议做敌人意图，非 9 条必显 |
| O-38 | 目标语义：单体=候选池选一（#178/#179） | T-M5-02（AI 选一）、T-M5-06 #5、T-M5-07 ③b：UI/AI 目标选择落地 |
