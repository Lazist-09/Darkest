...
| 里程碑判据 | 状态 |
|---|---|
| #1/#2 编辑器可用/无导入错误 | ✅ **本机补验（2026-09-09）**：Godot **4.6.1 stable mono**（`E:\Godot_v4.6.1-stable_mono_win64\Godot_v4.6.1-stable_mono_win64.exe`，出处：`.godot/mono/metadata/ide_messaging_meta.txt` 实况 + `--version` 复核）`--headless --path darkest --import` → first_scan_filesystem / update_scripts_classes / loading_editor_layout 全部 DONE、退出码 0，**无项目导入错误/SCRIPT ERROR/error CS**；编辑器已识别 project.godot（features 4.6）并加载 .godot/mono/temp 的 Darkest.dll；res:// 根第二 csproj（Darkest.Tests）未引发误导入。噪音（非项目）：向 `%APPDATA%/Godot/editor_settings-4.6.tres` 写设置被沙箱拒绝、root 证书库读取告警。编辑器侧 `--build-solutions` 回调在本机失败 = 离线 net8 restore（§5.1 事实 1），经 CI/.NET 8 机器复验。**注意**：Godot C# 插件保存 csproj 时把 `TargetFramework` 间接取值【实化为字面量 net8.0】（留 .old 备份）——已还原并纳入 §5.4 维护纪律 |
...
- T-M0-01 要点1「首个 csproj/sln 由编辑器生成」：本机初判无编辑器，按编辑器生成形状手写并记录；**2026-09-09 编辑器冒烟补验**（见 §5.2 #1/#2 行）。**维护纪律**：Godot 4.6 C# 插件保存 csproj 时会把 `TargetFramework` 的间接取值实化为字面量 net8.0（并生成 `Darkest.csproj.old` 备份）；一旦被实化，本机 net10 覆盖即失效 → 还原该行为 `$(DarkestTargetFramework)` 并删除 `.old`（工程默认与 CI 不受影响）。`*.csproj.old` 已入 darkest/.gitignore。
