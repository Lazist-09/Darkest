...
> **上游**：[设计] doc/modules/skill.md（§1~§6）/ skill_data.md（§1~§6）/ character.md（§1、§2、§7）· doc/GDD.md §0.3、§1.2 · doc/modules/glossary.md §2、§7.2 · **决策引用**：#23、#127、#134、#136、#139、#140、#143、#149、#153、#156、#157、#165（**v0.44 增补：#178/#179 目标语义，O-38**）
...
| T-M3-05 | 目标解析与候选池（#178/#179） | 范围=可选池：单体选一 / AOE 全中 / any_ally 选一 / team 全体；双段同目标 | scripts/gameplay/sim/skill/SkillTargetResolver.cs |
...
### T-M3-05 目标解析与候选池（#178/#179：范围=可选池，单体选一）
- **上游**：[设计] doc/GDD.md §1.2（**v0.44：#178/#179 范围=可选池**）/§1.5（敌方 4 位无支援、空位不打空）；doc/modules/skill.md §2 字段3（目标语义红字警示）；doc/modules/glossary.md §2（部分非空/技能灰显）；doc/architecture/data_schema.md §3.2 target 生效语义（v0.44 派生规则）/§4.2 **P11** · 决策：#23、#73、#77、#105、**#178、#179、O-38**
...
- **要点**：
  ...
  2. **目标数量语义（#178/#179，v0.44，推翻旧"范围全命中"读法）**：命中数量**不经数据声明**、由引擎按 `scope` + `tags.aoe` 派生：
     ...
     - `any_ally`（急救/战场鼓舞）→ 候选池 = 全体友方非空（含支援位 5、6），**选一**单体；
     - `team` → 全体友方；`self` → 自身；`adjacent_ally_and_self` → 自身 + 相邻（多目标刻意设计，#159）。
     - 范围内**全部为空** → 可用性 NoTarget（T-M3-03，UI 灰显"范围内没有目标"）；**部分非空** → 单体在非空中选一、AOE 打全部非空（不浪费、不落空，#23 兼容）。
     - **双段（0.55×2 / 0.5×2）= 同一目标两段，目标只选一次（#179）**；数据零改动（P11 校验兜底，data_schema §4.2）。
...
- **完成判据（可测）**：
  - 构造快照"敌 1 空 / 敌 2 有角色 / 敌 3 障碍 / 敌 4 角色"：范围技能目标 敌 1~3 → 候选池 [2,3]（跳过空 1）；目标仅敌 1 且空 → NoTarget；
  - **选一断言（#178）**：劈砍（target 敌 1、2 均非空、无 aoe）→ 候选池 [1,2]；执行层选中 2 → **仅产生目标 2 的伤害事件**（目标 1 无事件）；精准射击（target 敌 1~4）→ 候选池 = 全部非空位、执行层仅结算 1 个；
  - **AOE 全中断言（#178）**：横扫（敌 1~3 满非空）→ 3 个目标各产生伤害事件；精神震荡（我 1~2）同理；
  - **any_ally 选一断言**：急救在满编 6 人快照 → 候选池 = 6 个我方非空位、执行层仅结算选中 1 人（**不再是全员各回 12**）；
...
- **上游**：[设计] doc/modules/combat_math.md §5.2（先伤害→再士气→再虚弱/死门）；doc/architecture/blueprint.md §5b（工作次序草案：附加/位移在本技能全部目标结算后执行）/§8（管线次序）；data_schema.md §2.3（随机判定写法）/§3.2（段/效果/位移子结构）· 决策：#23、#117、**#178、#179**；O-12、O-13、O-24、O-21、O-38
...
- **要点**：
  1. **骨架固定形状**（伪代码级契约，具体实现归主程序）：
     ...
     ExecuteSkill(snapshot, skillId, 目标选择):
       ...
       pool   = SkillTargetResolver.Resolve(...)            // T-M3-05：候选池（槽号升序、含障碍）
       chosen = 选目标(pool)                                 // #178/#179：单体/any_ally→池内选一；aoe→全部非空；team/adjacent→全体；双段→同目标两段
       foreach t in chosen:                                 // 灰显/NoTarget 时 chosen 为空 → 短路
           DamagePipeline.Execute(施法者, t, skill.damage)  // M2：命中→暴击→(同一目标多段逐段)→伤害→士气→虚弱/死门
       全部目标结算完成后:
  ...
  2. **多段逐段（O-13 + #179 已定形）**：双连击/连射每段 = 一次独立"伤害实例"，各段分别走命中/暴击/减免/护盾判定；**目标只选一次（#179）——先选目标、再对同一目标逐段结算**；段间不重建目标列表；每段抽取经 IRngProvider（blueprint §8 唯一随机出口 + DrawCount 审计）。
  ...
  8. **v0.44 改造清单（#178/#179，O-38）——主程序必改三处**：
     - **SkillTargetResolver**：从"返回范围内全部非空槽"改为"返回**候选池**"；提供选一入口契约（候选池由调用方读取，`BattleCommand` 最终携带选定目标槽）；
     - **SkillExecutor**：按技能类型结算——单体/any_ally **只结算被选中的 1 个目标**（急救不再是"全体各回 12"）、AOE 结算全部非空、team/adjacent 全体；伤害路径与支援路径两处同口径；
     - **配合**：P11 校验（aoe 标签约束）+ 单目标结算断言（劈砍恰好 1 目标、敌方精准射击恰好 1 目标、急救恰好 1 目标）；数据 JSON 零改动。
- **完成判据（可测）**：
...
### 9.4 v0.44 改造待办（#178/#179 目标语义，O-38 —— 主程序按此重跑本里程碑判据）

| 项 | 内容 | 判据 |
|---|---|---|
| 1 | `SkillTargetResolver` 改**候选池**语义 + 选一入口契约 | 候选池正确（含障碍、槽升序）；单体技能不再返回"全命中" |
| 2 | `SkillExecutor` 单目标/多目标结算按 #178/#179（单体/any_ally 只结算 1 个；aoe 全中；team/adjacent 全体；双段同目标两段） | 劈砍恰 1 目标、急救恰 1 目标、横扫 3 目标、双连击同目标两段事件 |
