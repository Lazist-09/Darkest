...
- 冲突处理：上游文档冲突时以 **`modules/` 为准**（_conventions.md §1 / README §6 条 2）；模块内部仍有歧义或跨模块矛盾而影响本 schema 处，一律记入 `open_issues.md` B 节（权威编号 **O-11~O-31**，本文件 §7 同步归档），**不得在本文件内替策划拍板**。
...
| 2 | 撤退成功率具体数字 | 运行时按 GDD §3.6 混合式（f 未定 → O-11）；显示代价读 tuning `retreat` |
...
> 生效语义（GDD §4 / formation §4）：范围内**全部为空 → 灰显不可释放**；**部分非空 → 只对非空位置生效**（不浪费、不落空）。此为引擎规则，不入数据。
...
> 按 _conventions §5：O-01~O-10 已被策划 README §5 的 10 条占用；以下为 schema 侧新增疑问，**编号即 open_issues.md B 节的权威编号（O-11~O-31）**，标注【策划拍板】/【架构裁定】/【实现期待定】与阻塞范围。**本文不改 open_issues.md**，以下仅供架构师归档与任务卡引用。
...
| O-11 | 【架构裁定】撤退成功率的 f 函数 | GDD §3.6"基础成功率 = f(速度对比)"，f 未定义；ui_spec §2 要求显示具体数字（例 68%） | 撤退按钮数字、撤退结算（数据非阻塞，功能阻塞） | 待实现期/策划定义 f（±10% 随机 #46） |
...
| O-13 | 【架构裁定】多段倍率（0.55×2 / 0.5×2）各段是否分别过命中/暴击/护盾 | skill_data 仅定义"两段各 0.55"；combat_math §8.1 护盾"一次 AOE 算一次攻击"，多段是否算多次攻击未定义 | 双连击/连射的逐段结算与护盾消耗 | 切片无"多段 vs 护盾"同时在场组合，低优先 |
| O-31 | 【架构裁定】威吓箭伤害表达式"精神 12×0.5"与远程射手攻击 13 不符 | enemy.md §5.3 写 12×0.5，但远程射手攻击 = 13（enemy §5.2 / #154）；按 combat_math §2 通用公式应为攻击者攻击 × 倍率 | 威吓箭实际伤害数字（倍率 0.5 以 skill_data 为准，不阻塞） | 疑为笔误，待策划确认；数据按 0.5 倍率 + 攻击 13 结算 |
